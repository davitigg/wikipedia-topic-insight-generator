...
async def analyze_topic(
    ...
    analysis_result = await analysis_service.analyze_topic(topic_name)
...
async def get_analyses(
    ...
    results = await analysis_service.get_analyses(topic_name)
    return results
...
async def get_specific_analysis(analysis_id: str = Path(..., description="The unique ID of the analysis result")):
    result = await analysis_service.get_specific_analysis(analysis_id)
    if result is None:
...
async def delete_analyses(
    ...
    deleted_count = await analysis_service.delete_analyses(topic_name)
    if deleted_count == 0:
...
async def delete_specific_analysis(
    ...
    result = await analysis_service.delete_specific_analysis(analysis_id)
    if result is False:
        raise HTTPException(status_code=404, detail="Analysis result not found")
...
async def get_topics():
    topics = await analysis_service.get_topics()
    return topics
